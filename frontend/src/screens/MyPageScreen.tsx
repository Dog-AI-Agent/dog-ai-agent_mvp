...
// ── 입력 필드 공통 컴포넌트 ──
const Field = ({
  label, value, onChangeText, editable = true, placeholder, secureTextEntry,
}: {
  label: string; value: string; onChangeText?: (v: string) => void;
  editable?: boolean; placeholder?: string; secureTextEntry?: boolean;
}) => (
  <View style={{ gap: 4 }}>
    ...
    <TextInput
      style={{
        ...
        fontSize: 14, color: editable ? "#1f2937" : "#9ca3af",
        backgroundColor: editable ? "#ffffff" : "#f9fafb",
      }}
      value={value} onChangeText={onChangeText} editable={editable}
      placeholder={placeholder} placeholderTextColor="#9ca3af" secureTextEntry={secureTextEntry}
    />
  </View>
);
...
const UserInfoTab = () => {
  ...
  const [form, setForm] = useState({
    ...
    birth_date: user?.birth_date ?? "", address: user?.address ?? "",
  });
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await updateMe({
        name: form.name || undefined, nickname: form.nickname || undefined,
        birth_date: form.birth_date || undefined, address: form.address || undefined,
      });
      updateUser(updated); setEditing(false); setSuccess("정보가 수정되었습니다.");
    } catch (e: any) {
  ...
  const handleCancel = () => {
    setForm({ name: user?.name ?? "", nickname: user?.nickname ?? "",
      birth_date: user?.birth_date ?? "", address: user?.address ?? "" });
      ...
      {editing ? (
        <View style={{ flexDirection: "row", gap: 10 }}>
          ...
          <Pressable onPress={handleSave} disabled={loading}
            style={({ pressed }) => ({ flex: 2, backgroundColor: pressed ? PRIMARY_PRESSED : PRIMARY, borderRadius: 12, paddingVertical: 12, alignItems: "center" })}>
            {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={{ fontSize: 14, color: "#fff", fontWeight: "700" }}>저장</Text>}
          </Pressable>
        </View>
      ) : (
        <Pressable onPress={() => { setEditing(true); setSuccess(""); }}
          style={({ pressed }) => ({ backgroundColor: pressed ? PRIMARY_PRESSED : PRIMARY, borderRadius: 12, paddingVertical: 12, alignItems: "center" })}>
...
const DogHistoryTab = () => {
  ...
  const isAllPageSelected = pageItems.length > 0 && pageItems.every((i) => selectedIds.includes(i.history_id));
  ...
  const toggleAllPage = () => {
    if (isAllPageSelected) {
      ...
      setSelectedIds((prev) => {
        const merged = [...prev];
        pageIds.forEach((id) => { if (!merged.includes(id)) merged.push(id); });
        return merged;
      });
    }
  };
  ...
  if (loading) {
    return (
      <View style={{ alignItems: "center", marginTop: 60 }}>
        ...
        <Text style={{ marginTop: 12, fontSize: 14, color: "#9ca3af" }}>불러오는 중...</Text>
      </View>
    );
  }
  ...
  if (history.length === 0) {
    return (
      <View style={{ alignItems: "center", marginTop: 60, gap: 12 }}>
        ...
        <Text style={{ fontSize: 16, fontWeight: "700", color: "#1f2937" }}>분석 기록이 없습니다</Text>
  ...
  return (
    <View style={{ gap: 14 }}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        {/* 왼쪽: 건수 or 선택 현황 */}
        {!selectionMode ? (
          <Text style={{ fontSize: 13, color: "#9ca3af" }}>총 {history.length}건의 분석 기록</Text>
        ) : (
          <Text style={{ fontSize: 13, fontWeight: "600", color: "#1f2937" }}>
            {selectedIds.length > 0 ? `${selectedIds.length}개 선택됨` : "항목을 선택하세요"}
          </Text>
        )}
        ...
        <View style={{ flexDirection: "row", gap: 8 }}>
          {selectionMode && (
            ...
            >
              {deleting ? (
                <ActivityIndicator color="#fff" size="small" style={{ width: 40 }} />
              ) : (
                <Text style={{
          ...
          >
            <Text style={{ fontSize: 13, fontWeight: "600", color: selectionMode ? "#6b7280" : PRIMARY }}>
              {selectionMode ? "취소" : "선택"}
            </Text>
          </Pressable>
        </View>
      ...
      {deleteError ? (
        <View style={{ backgroundColor: "#fef2f2", borderRadius: 10, padding: 10 }}>
          <Text style={{ fontSize: 13, color: "#ef4444" }}>⚠️ {deleteError}</Text>
        </View>
      ) : null}
      ...
      {selectionMode && (
        ...
        >
          <View style={{
            ...
            {isAllPageSelected && <Text style={{ color: "#fff", fontSize: 12, fontWeight: "700" }}>✓</Text>}
          </View>
      ...
      {pageItems.map((item) => {
        ...
        return (
          <Pressable
            ...
            style={({ pressed }) => ({
              backgroundColor: isSelected ? "#eef0fd" : (pressed ? "#f8f9fa" : "#fff"),
              borderRadius: 16,
          ...
          >
            ...
            <View style={{ position: "relative" }}>
              {item.image_url ? (
                ...
                <View style={{ width: "100%", aspectRatio: 4 / 3, backgroundColor: PRIMARY_LIGHT,
                  alignItems: "center", justifyContent: "center" }}>
                  <Text style={{ fontSize: 48 }}>🐶</Text>
                  <Text style={{ fontSize: 12, color: "#9ca3af", marginTop: 6 }}>이미지 없음</Text>
                </View>
              )}
              {/* 체크 오버레이 */}
              {selectionMode && (
            ...
            <View style={{ padding: 14, gap: 8 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <Text style={{ fontSize: 16, fontWeight: "700", color: "#1f2937" }}>{item.breed_name_ko}</Text>
                {item.is_mixed_breed && (
                  <View style={{ backgroundColor: "#fef3c7", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3 }}>
                    <Text style={{ fontSize: 12, color: "#d97706", fontWeight: "600" }}>믹스견</Text>
                  </View>
                )}
              </View>
              {item.breed_name_en
                ? <Text style={{ fontSize: 13, color: "#6b7280" }}>{item.breed_name_en}</Text>
                : null}
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                {item.confidence != null ? (
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    <View style={{ height: 6, width: 80, backgroundColor: "#e5e7eb", borderRadius: 4, overflow: "hidden" }}>
                      <View style={{ height: "100%", width: `${Math.round(item.confidence * 100)}%`,
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                  <Text style={{ fontSize: 12, color: "#9ca3af" }}>{formatDate(item.created_at)}</Text>
      ...
      {totalPages > 1 && (
        <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 4 }}>
          <Pressable onPress={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}
            style={{ width: 36, height: 36, borderRadius: 18, borderWidth: 1,
              borderColor: page === 0 ? "#e5e7eb" : PRIMARY,
              alignItems: "center", justifyContent: "center", backgroundColor: "#fff", opacity: page === 0 ? 0.4 : 1 }}>
            <Text style={{ fontSize: 16, color: page === 0 ? "#9ca3af" : PRIMARY }}>‹</Text>
          </Pressable>
          {Array.from({ length: totalPages }, (_, i) => (
            <Pressable key={i} onPress={() => setPage(i)}
              style={{ width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center",
                backgroundColor: page === i ? PRIMARY : "#fff",
                borderWidth: 1, borderColor: page === i ? PRIMARY : "#e5e7eb" }}>
              <Text style={{ fontSize: 14, fontWeight: "600", color: page === i ? "#fff" : "#6b7280" }}>{i + 1}</Text>
            </Pressable>
          ))}
          <Pressable onPress={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1}
            style={{ width: 36, height: 36, borderRadius: 18, borderWidth: 1,
              ...
              alignItems: "center", justifyContent: "center", backgroundColor: "#fff", opacity: page === totalPages - 1 ? 0.4 : 1 }}>
            <Text style={{ fontSize: 16, color: page === totalPages - 1 ? "#9ca3af" : PRIMARY }}>›</Text>
          </Pressable>
        </View>
      )}
        ...
        {(["user", "dog"] as Tab[]).map((t) => (
          <Pressable key={t} onPress={() => setTab(t)}
            style={{ flex: 1, paddingVertical: 14, alignItems: "center",
              borderBottomWidth: 2, borderBottomColor: tab === t ? PRIMARY : "transparent" }}>
