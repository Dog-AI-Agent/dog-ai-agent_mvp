...
// - 믹스견: 위험도 한 단계 하향
// - isGuest: 비회원 모드 (API 저장 스킵, 로그인 유도)
// - historyId: 히스토리에서 열기 (재저장 스킵)
// - illustrationUrl: AI 일러스트 기능
// ============================================================
...
import { updateMyDog, saveAnalysis, createMyDog, generateIllustration } from "../api/users";
...
const DiseaseSection = ({ title, diseases, color, defaultOpen }: {
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      <Pressable
        onPress={() => { LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); setOpen((v) => !v); }}
        style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: color + "18", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 11, borderLeftWidth: 4, borderLeftColor: color }}
      >
      ...
      {open && (
        <View style={{ gap: 6, marginTop: 6 }}>
          {diseases.map((d) => (
            <View key={d.disease_id} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#f8f9fa", borderRadius: 10, paddingHorizontal: 14, paddingVertical: 11 }}>
...
const BreedResultScreen = ({ navigation, route }: Props) => {
  ...
  const [analysisId, setAnalysisId] = useState<string | null>(historyId ?? null);
  const [illustrationUrl, setIllustrationUrl] = useState<string | null>(paramIllustrationUrl ?? null);
  ...
  useEffect(() => {
    ...
    // 비회원이거나 히스토리에서 열면 저장 스킵
    if (!isGuest && !isFromHistory) {
      ...
      saveAnalysis({
        ...
        .then((saved: any) => setAnalysisId(saved?.history_id ?? null))
        .catch(() => {});
    }
  ...
  const sortedDiseases: DisplayDisease[] = [...(breedDetail?.diseases ?? [])]
    ...
    .map((d) => ({ ...d, display_risk: isPurebred ? d.risk_level : downgradeRisk(d.risk_level) }));
  ...
  if (loading) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
        <UserHeader />
        {canGoBack && (
          <Pressable onPress={() => navigation.goBack()} style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 20, paddingVertical: 10 }}>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      <UserHeader />
      {canGoBack && (
        <Pressable onPress={() => navigation.goBack()} style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 20, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#f3f4f6" }}>
          <Text style={{ fontSize: 20, color: "#6b7280" }}>←</Text>
          <Text style={{ fontSize: 14, color: "#6b7280" }}>뒤로가기</Text>
        </Pressable>
      )}
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingVertical: 16, gap: 16, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        <Text style={{ textAlign: "center", fontSize: 22, fontWeight: "800", color: "#1f2937" }}>품종 분석 결과</Text>

        {(gradcamUri || imageUri) && (
          <Image source={{ uri: gradcamUri ?? imageUri }} style={{ width: "100%", borderRadius: 16, aspectRatio: 1 }} resizeMode="cover" />
        )}
        ...
        <View style={{ backgroundColor: "#f8f9fa", borderRadius: 16, padding: 18, gap: 12 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
            ...
            <View style={{ alignItems: "flex-end", gap: 4 }}>
              ...
              <View style={{ borderRadius: 20, backgroundColor: isPurebred ? "#dbeafe" : "#fef9c3", paddingHorizontal: 10, paddingVertical: 3 }}>
          ...
          {result.top_k_predictions.length > 0 && <DonutChart predictions={result.top_k_predictions} />}
        </View>

        {breedDetail && (
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
            <InfoCard label="크기" value={breedDetail.size_category ? ({ small: "소형", medium: "중형", large: "대형", giant: "초대형" } as Record<string, string>)[breedDetail.size_category] || breedDetail.size_category : "—"} />
        ...
        {!isGuest && analysisId && result.breed_name_en && (
          <View style={{ backgroundColor: "#faf5ff", borderRadius: 16, padding: 18, alignItems: "center", gap: 12 }}>
            ...
            {illustrationUrl ? (
              <Image source={{ uri: illustrationUrl }} style={{ width: "100%", aspectRatio: 1, borderRadius: 14 }} resizeMode="cover" />
            ) : illustrationLoading ? (
              <View style={{ alignItems: "center", gap: 8, paddingVertical: 20 }}>
                <ActivityIndicator size="large" color="#7c3aed" />
                <Text style={{ fontSize: 13, color: "#7c3aed" }}>AI가 그리고 있어요... (15~30초)</Text>
              </View>
            ) : (
              <Pressable onPress={handleGenerateIllustration} style={{ backgroundColor: "#7c3aed", borderRadius: 12, paddingVertical: 12, paddingHorizontal: 24 }}>
                <Text style={{ fontSize: 14, fontWeight: "600", color: "#fff" }}>귀여운 일러스트 생성하기</Text>
              </Pressable>
            )}
          </View>
        )}
          ...
          <View style={{ gap: 4 }}>
            ...
            {!isPurebred && (
              <View style={{ backgroundColor: "#f0f9ff", borderRadius: 10, padding: 10, marginBottom: 4 }}>
                <Text style={{ fontSize: 12, color: "#0369a1" }}>믹스견 추정으로 유전병 위험도가 한 단계 낮게 조정되었습니다.</Text>
              </View>
            )}
        ...
        {breedDetail && sortedDiseases.length === 0 && <EmptyState message="이 품종에 대한 유전병 데이터가 아직 없습니다." />}
      </ScrollView>

      <View style={{ paddingHorizontal: 20, paddingVertical: 12, paddingBottom: 20, gap: 8, backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: "#f3f4f6" }}>
        {result.breed_id && (
          isGuest ? (
            <Pressable style={{ backgroundColor: "#4361ee", borderRadius: 14, paddingVertical: 14 }} onPress={() => navigation.navigate("Login")}>
              <Text style={{ textAlign: "center", fontSize: 15, fontWeight: "700", color: "#fff" }}>회원가입하고 맞춤 추천 이용하기 →</Text>
            </Pressable>
          ) : (
            <Pressable style={{ backgroundColor: "#4361ee", borderRadius: 14, paddingVertical: 14 }} onPress={() => navigation.navigate("Recommendation", { breedId: result.breed_id!, breedNameKo: result.breed_name_ko })}>
              <Text style={{ textAlign: "center", fontSize: 15, fontWeight: "700", color: "#fff" }}>맞춤 추천 보기</Text>
            </Pressable>
          )
        )}
        <Pressable style={{ backgroundColor: "#f3f4f6", borderRadius: 14, paddingVertical: 12 }} onPress={() => navigation.navigate(isGuest ? "GuestUpload" : "Upload")}>
