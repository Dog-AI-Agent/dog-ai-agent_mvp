...
type Props = NativeStackScreenProps<RootStackParamList, "CommunityPostEdit">;
...
const CommunityPostEditScreen = ({ navigation, route }: Props) => {
  ...
  // 이미지: 기존 이미지 (서버에 있는 것) + 새로 추가할 이미지 URI
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setIsRecipe(post.category === "recipe");
        setExistingImages(post.images || []);
        if (post.category === "recipe" && post.recipe_data) {
          setIngredients(post.recipe_data.ingredients || []);
          setSteps(post.recipe_data.steps || []);
        }
      } catch (e: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const recipeData =
        isRecipe && (ingredients.length > 0 || steps.length > 0)
          ? { ingredients, steps }
          : null;
      ...
      if (newImageUris.length > 0) {
        await uploadPostImages(postId, newImageUris);
      }
